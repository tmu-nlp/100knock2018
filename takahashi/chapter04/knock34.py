...
with open('../data/neko.txt.mecab', 'r', encoding='utf-8') as f:
    for line in f:
        if line == 'EOS\n':
            ...
            if flg == 0 and pos == '名詞' and pos1 != '非自立':
                phrase += surface
                flg = 1
            elif flg == 1 and pos == '助詞' and pos1 == '連体化':
                phrase += surface
                flg = 2
            elif flg == 2 and pos == '名詞' and pos1 != '非自立':
                phrase += surface
                flg = 3
            else:
                phrase = ''
                flg = 0
            if flg == 3:
                noun_consolidation.extend([phrase])
                phrase = ''
                flg = 0

print(len(noun_consolidation))
print(noun_consolidation)
