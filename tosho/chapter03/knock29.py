...
def retrieve_flag_url(info):
    '''
    https://commons.wikimedia.org/w/api.php?action=query&titles=File%3AAlbert+Einstein+Head.jpg&prop=imageinfo&iiprop=url&iiurlwidth=220&format=json
    参考
    http://toricor.hatenablog.com/entry/2016/01/16/160406
    '''
    svg_filename = info['国旗画像']
    if len(svg_filename) <= 0:
        return info

    url = []
    url.append('https://commons.wikimedia.org/w/api.php?')
    url.append('action=query&')
    url.append('titles=' + html.escape('File:' + svg_filename) + '&')
    url.append('prop=imageinfo&')
    url.append('iiprop=url&')
    ...
    data = r.json()
    act_url = data['query']['pages'].popitem()[1]['imageinfo'][0]['url']
